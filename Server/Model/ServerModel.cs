                ...
                for (int i = 1; i <= bombToExplode.Power; i++)
                {
                    ...
                    switch (direction)
                    {
                        ...
                        case 2:
                            ...
                            //right
                        default:
     impacted        tempLi                              x => x.Position.X == bombToExplode.Position.X + i
                                         && x.Position.Y == bombToExplode.Position.Y).ToList()impacted                 CheckBomb(tempList);
                          }
                    //iimpacted   if (tempList.Count == 0)
                        continue;
                    //if we encountered a wall undestructible don't need to go further in the current direction
                    if (IsUndestructible(tempList))
                      
            //foreach Impacted bomb  break;
